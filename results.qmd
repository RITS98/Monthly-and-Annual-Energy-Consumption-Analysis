...
-   Mid-2000s to Present: There is a sharp decline in net imports, eventually turning negative. This indicates that the U.S. became a net exporter of primary energy. Factors contributing to this include increased domestic energy production (especially from gas and oil), improved energy efficiency, and shifts towards renewable energy sources. (Source: [U.S. Energy Independence](https://en.wikipedia.org/wiki/United_States_energy_independence))
...
**Consumption Trends**  
Total Primary Energy Consumption shows a steady upward trajectory from around 6.5 Quadrillion Btu to approximately 7.5 Quadrillion Btu each month starting from January 1973 and ending on August 2024. Notable seasonal fluctuations appear throughout the timeline with regular peaks and troughs. The overall consumption pattern demonstrates consistent growth despite short-term variations
...
**Coal (Red)**
- The red bars at the top show coal production. It stayed fairly steady increase from 1949 to 1980, then nearly stabilized after 1980 but eventually decreased around late 2010s and the trend continues till now.
...
The dependency on fossil fuels especially on natural gas is very pronounced in U.S. 
...
-   Shows rapid recent growth due advancement in technology and good adoption by the government and people.  
...
The visualization clearly demonstrates the transition toward a more diverse renewable energy portfolio, with newer technologies like wind and solar becoming increasingly important contributors to the overall energy mix. But the difference between fossil fuels and renewable is very wide. Serious policy change and incentives are required to make the transition to renewable energy. 
...
Supporting renewable energy and reducing reliance on fossil fuels can be achieved by:
	1.	**Investing in Clean Technologies**: Allocate funding for solar, wind, hydroelectric, and other renewable energy projects.  
	2.	**Government Policies and Incentives**: Implement subsidies, tax credits, and carbon pricing to encourage renewable energy adoption.  
	3.	**Grid Modernization**: Upgrade infrastructure to support energy storage and integrate decentralized renewable sources.  
	4.	**Promoting Energy Efficiency**: Encourage the use of energy-efficient appliances and buildings to reduce overall demand.  
	5.	**Public Awareness**: Educate communities about the benefits of renewables and sustainable practices to drive grassroots support.  
...
The green line represents energy consumption, which follows a distinct seasonal pattern. Energy use regularly peaks at around 3,500 trillion BTU and drops to about 2,000 trillion BTU throughout each year. These peaks correspond to winter months when heating demands are highest, while valleys represent periods of lower energy usage, typically during milder seasons.
...
There are certain dips in the graph like in 1980s, 2008 and 2020. This corresponding to major shifts in industrial sectors due to global crisis like financial crisis of 2008, covid pandemic, etc.
...
```
This map displays the percentage distribution of energy production across the United States, including Alaska and Hawaii. The color gradient ranges from light orange (lowest percentage) to dark red (highest percentage), with values ranging from 0% to 25% as of 2022.
...
- The Western and eastern states show minimal energy production.  
...
```
This map visualization shows the energy expenditure per capita across the United States, including Alaska and Hawaii. The data is represented through a color gradient, where darker blue indicates higher expenditure and lighter blue shows lower expenditure.

Alaska stands out with the highest energy expenditure per capita, shown in deep teal color, suggesting spending of more than $12,000 per person. This is due to very low population.

Several states in the northern central region (including North Dakota and Wyoming) show moderately high energy expenditure, indicated by medium-dark blue shading.

Texas and Louisiana in the South also display elevated energy expenditure levels compared to surrounding states

Coastal and most inland states show lighter blue shading, indicating lower per capita energy expenditure
...
co2 <- energy_data_annual |> select(
  `Year`,
  `Coal, Including Coal Coke Net Imports, CO2 Emissions (Million Metric Tons of Carbon Dioxide)`,
  `Natural Gas, Excluding Supplemental Gaseous Fuels, CO2 Emissions (Million Metric Tons of Carbon Dioxide)`,     
  `Aviation Gasoline CO2 Emissions (Million Metric Tons of Carbon Dioxide)`,                                      
  `Kerosene CO2 Emissions (Million Metric Tons of Carbon Dioxide)`,                                               
  `Motor Gasoline, Excluding Ethanol, CO2 Emissions (Million Metric Tons of Carbon Dioxide)`,
  `Petroleum, Excluding Biofuels, CO2 Emissions (Million Metric Tons of Carbon Dioxide)`,
  `Other Petroleum Products CO2 Emissions (Million Metric Tons of Carbon Dioxide)`,                               
) |> rename(
  coal = `Coal, Including Coal Coke Net Imports, CO2 Emissions (Million Metric Tons of Carbon Dioxide)`,
  natural_gas = `Natural Gas, Excluding Supplemental Gaseous Fuels, CO2 Emissions (Million Metric Tons of Carbon Dioxide)`,
  aviation = `Aviation Gasoline CO2 Emissions (Million Metric Tons of Carbon Dioxide)`,
  kerosene = `Kerosene CO2 Emissions (Million Metric Tons of Carbon Dioxide)`,
  motor_gasoline = `Motor Gasoline, Excluding Ethanol, CO2 Emissions (Million Metric Tons of Carbon Dioxide)`,
  petroleum = `Petroleum, Excluding Biofuels, CO2 Emissions (Million Metric Tons of Carbon Dioxide)`,
  other = `Other Petroleum Products CO2 Emissions (Million Metric Tons of Carbon Dioxide)`
) |> drop_na() |>
  pivot_longer(cols = -Year, names_to='fuel_type', values_to='emissions')

pe_prod <- energy_data_annual |> select(
  Year,
  `Total Fossil Fuels Production (Quadrillion Btu)`,
  `Nuclear Electric Power Production (Quadrillion Btu)`,
  `Total Renewable Energy Production (Quadrillion Btu)`
) |> rename(
  fossil_fuel = `Total Fossil Fuels Production (Quadrillion Btu)`,
  nuclear_energy = `Nuclear Electric Power Production (Quadrillion Btu)`,
  renewable_energy = `Total Renewable Energy Production (Quadrillion Btu)`
) |> drop_na() |>
  pivot_longer(
    cols = -Year, names_to = 'energy_type', values_to='production'
  )
write.csv(pe_prod, './data_source/pe_production.csv')
write.csv(co2, './data_source/co2_emissions_by_fuel_type.csv')

ggplot(co2, aes(x=Year, y=emissions, color=fuel_type)) +
  geom_line(size=1) + 
...
This graph shows CO2 emissions trends by different fuel types from approximately 1973 to 2023, measured in million metric tons. Several notable patterns emerge:

**Petroleum** consistently remains the highest emitter throughout the period, showing significant fluctuations with peaks around 1980 and 2005-2010, followed by a notable decline and slight recovery toward 2020.

**Coal** emissions (shown in blue) demonstrate a steady increase from 1975 until around 2005, after which they experience a dramatic decline, particularly steep after 2010. By 2020, coal emissions have dropped to their lowest levels in the recorded period.

**Natural Gas** emissions (orange line) show a general upward trend, particularly accelerating after 2005. By the end of the period, natural gas emissions have surpassed coal, becoming the second-largest source of CO2 emissions.

**Motor Gasoline** (purple line) maintains relatively stable emissions throughout the period, with a slight increase during the 1990s and 2000s, followed by a modest decline.

**Other** sources (yellow line) and **Aviation/Kerosene** (red and green lines) contribute relatively minor amounts to overall emissions, remaining fairly constant throughout the period.

**Key Transitions**: The most significant change in the data is the dramatic shift in the energy landscape after 2010, marked by:  
- A sharp decline in coal emissions.  
- A concurrent rise in natural gas emissions.  
- A moderate decrease in petroleum emissions.  

The overall emission patterns have significant implications for climate change:  
- The shift from coal to natural gas represents a partial improvement, as natural gas produces about half the CO2 emissions per unit of energy compared to coal.  
- Despite the decline in coal usage, the total emissions remain high enough to contribute significantly to global warming.  
- The persistent high levels of petroleum emissions, combined with rising natural gas usage, continue to pose challenges for meeting climate goals. 


This comprehensive emissions profile suggests that while there have been some positive changes in the energy mix, particularly the reduction in coal usage, the United States continues to produce substantial CO2 emissions that contribute to global climate change. Further reductions across all fuel types would be necessary to meaningfully address climate change concerns.
